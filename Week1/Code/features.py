...
if __name__ == '__main__':
    ...
    train_data = pd.read_table(train_path,names=["id","sent1","sent2","score"],header = None)
    
    test_path = r'C:\Users\Administrator\Desktop\AI_Camp\Tal_SenEvl_test_62KB.txt'
    test_data = pd.read_table(test_path,names=["id","sent1","sent2"],header = None)
    ...
    train_text['sentence2'] = [preProcess(x) for x in list(train_data['sent2'])]
    
    test_text = pd.DataFrame()
    test_text['sentence1'] = [preProcess(x) for x in list(test_data['sent1'])]
    test_text['sentence2'] = [preProcess(x) for x in list(test_data['sent2'])]
    #==============================================================================
    # 特征提取
    #==============================================================================
