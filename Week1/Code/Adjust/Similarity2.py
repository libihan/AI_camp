...
if __name__ == '__main__':
    ...
    train_features['tfidf_features'],train_features['lsi_features']=getFeature(train_text1,train_text2)
    
    #==============================================================================
    # 模型训练
    #==============================================================================
    # 弱学习器迭代次数n_estimators,决策树最大深度max_depth,内部节点再划分所需最小样本数min_samples_split
#    parameters = {'n_estimators':[10,20,40,80], 'max_depth'::[5,10,15], 'min_samples_split': [50,100,150,200]}
    
    
    #(1)不管任何参数,用默认的 --- 看一下效果
    rf0 = RandomForestRegressor(oob_score=True, random_state=10)
    rf0.fit(train_features,train_labels)
    preds = rf0.predict(train_features)
    print('-----默认参数-----')
    print('袋外分数(泛化能力):%f'%rf0.oob_score_)
    print('MSE:%f'%pearsonr (list(train_labels), list(preds))[0])
    
    #(2)首先对弱学习器迭代次数n_estimators进行网格搜索
    ...
    gsearch1= GridSearchCV(estimator = RandomForestRegressor(min_samples_split=100,min_samples_leaf=20,\
                                                             ...
                                                             random_state=10),param_grid =param_test1, scoring='roc_auc',cv=5)
    gsearch1.fit(train_features,np.mat(train_labels).T)
